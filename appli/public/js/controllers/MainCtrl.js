angular.module('MainCtrl', []).controller('MainController', function($scope,$http) {
	...
	function ChangQuestion (numQuestion,pAnswer=null) {
		...
		$scope.fin = function () {
			$scope.question = "Thank you for participating in this survey. Your answer is invaluable to us to constitute the most representative sample of the companies in France.";
			$scope.theEnd = true;
			...
			document.getElementById("img").style.margin = "20% 20%";
    	...
    	//declaration des var pour le spe5
    		$scope.tfspe5 = "";
    		$scope.tfspe5;
    		$scope.tfspe5;
    		$scope.tfspe5;
    	...
    	else if ($scope.typeSel == "special3") {
    		$scope.spe3 = true;
    	}
    	else if ($scope.typeSel == "special4") {
    		$scope.spe4 = true;
    	}
    	else if ($scope.typeSel == "special5") {
	...
	$scope.next = function () {
		...
		console.log("data : "+$scope.blabla);
		//sauvegarde de la donnée dans un fichier json
		//$scope.storage += [$scope.numQuestion, $scope.blabla];
		

		if ($scope.t == true) {
			$scope.storage.push({q: $scope.numQuestion, a:$scope.blabla});
			console.log($scope.storage);
			ChangQuestion($scope.answers["text"]);
		}
		else if ($scope.cb == true) {
			$scope.storage.push({q: $scope.numQuestion, a:$scope.blabla});
			console.log($scope.storage);
			ChangQuestion($scope.answers["Yes, our paper is made from virgin paste"]);
		}
		else if ($scope.spe1 == true) {
			$scope.storage.push({q: $scope.numQuestion, a:$scope.blabla});
			console.log($scope.storage);
			ChangQuestion($scope.answers["No"]);
		}
		else if ($scope.spe2 == true) {
			$scope.storage.push({q: $scope.numQuestion, a:$scope.blabla});
			console.log($scope.storage);
			ChangQuestion($scope.answers["I don't know"]);
		}
		else if ($scope.spe3 == true) {
			$scope.storage.push({q: $scope.numQuestion, a:$scope.blabla});
			console.log($scope.storage);
			ChangQuestion($scope.answers["I don't know"]);
		}
		else if ($scope.spe4 == true) {
			$scope.storage.push({q: $scope.numQuestion, a:$scope.blabla});
			console.log($scope.storage);
			ChangQuestion($scope.answers["Yes, FSC"]);
		}
		else if ($scope.spe5 == true) {
			$scope.storage.push({q: $scope.numQuestion, a:$scope.tfspe5});
			console.log($scope.storage);
			ChangQuestion($scope.answers["Cartridges used a year"]);
		}
		else {
